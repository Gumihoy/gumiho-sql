...
public class SQLSubQueryTableReference extends AbstractSQLTableReference {
    ...
    public SQLSubQueryTableReference(ISQLSelectQuery subQuery, boolean as, ISQLIdentifier alias) {
        setSubQuery(subQuery);
        setAs(as);
        setAlias(alias);
    }
    ...
    public void cloneTo(SQLSubQueryTableReference x) {
        ...
        ISQLIdentifier aliasClone = this.alias;
        if (aliasClone != null) {
            x.setAlias(aliasClone);
        }

        for (ISQLExpr derived : this.columns) {
            ISQLExpr derivedClone = derived.clone();
            x.addColumn(derived);
        }
